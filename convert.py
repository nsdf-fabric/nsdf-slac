...
def ConnectS3(bucket_name=None, endpoint_url=None,aws_access_key_id=None, aws_secret_access_key=None, signature_version = None):
	...
	s3 = boto3.resource('s3', endpoint_url=endpoint_url, aws_access_key_id=aws_access_key_id, aws_secret_access_key=aws_secret_access_key, config=config,verify=False)
...
def MyJob(key):
	...
	if True:
		...
		print("Uploaded json file",gz_filename)
...
if __name__=="__main__":
	...
	for I,key in enumerate(files):
		key_noext=key.replace(".mid.gz","")
		done_filename=f"{key_noext}.extract.done"
		if os.path.isfile(done_filename): continue
		jobs.append((key,))
	...
	with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
		futures = [executor.submit(MyJob, *job) for job in jobs]
